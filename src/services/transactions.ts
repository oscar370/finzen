...
export async function getIncomes(yearMonth: string, search?: string, categoryId?: number) {
  const collection = db.transactions
    .where("[kind+yearMonth+date]")
    .between(["income", yearMonth, Dexie.minKey], ["income", yearMonth, Dexie.maxKey])
    .reverse();
...
export async function getExpenses(yearMonth: string, search?: string, categoryId?: number) {
  const collection = db.transactions
    .where("[kind+yearMonth+date]")
    .between(["expense", yearMonth, Dexie.minKey], ["expense", yearMonth, Dexie.maxKey])
    .reverse();
...
export async function updateTransaction(updates: Transaction) {
  ...
  await db.transaction("rw!", [db.transactions, db.categories, db.monthly_summaries], async () => {
